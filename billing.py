...
def main():
	try:
		...
		for i in range(0,no_of_item):
			...
			qty=raw_input("Enter the quantity  :")
			...
			print "Item_id\tItem_name\tItem_price\tItem_tax\tPerishable\tExp_date\t\tTotal"
			print "====================================================================================================================="
			for i in range(0,l):

				t=((data[i][2]*float(qty))+data[i][4])
				t1.append(t)
				print data[i][0],'\t',data[i][1],'\t''\t',data[i][2],'\t''\t',data[i][4],'\t''\t',data[i][5],'\t''\t',data[i][6],'\t''\t\t',t
				print "---------------------------------------------------------------------------------------------------------------------------"	
